...
public class ChangeWriteIndex implements Runnable {
	...
	private void ChangeIndexBase(CreateIndex single) {
		...
		try {
			...
			// 构造分词器
			single.analyzer = new IKAnalyzer(Version.LUCENE_45);
			// 初始化IndexWriter
			IndexWriterConfig cf = new IndexWriterConfig(Version.LUCENE_45, single.analyzer);
			Date dt = new Date();
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
			single.setToday(sdf.format(dt));

			Directory directory = FSDirectory.open(new File(ServerConfig.getInstance().getLocalIndexPath()
					+ single.getToday() + "/")); // FSDirectory(new
