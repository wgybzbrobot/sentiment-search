...
import zx.soft.sent.lucene.search.ServerConfig;
...
public class CreateIndex {
	 ...
	 * IK分词器
	 */
	public Analyzer analyzer = new IKAnalyzer(Version.LUCENE_45);
	/**
	 * 创建索引类实例
	 */
	...
	private CreateIndex() {
		...
		try {
			...
			// 构造分词器
			analyzer = new IKAnalyzer(Version.LUCENE_45);
			// 初始化IndexWriter
			IndexWriterConfig cf = new IndexWriterConfig(Version.LUCENE_45, analyzer);
			Date dt = new Date();
			SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
			setToday(sdf.format(dt));

			Directory directory = FSDirectory.open(new File(ServerConfig.getInstance().getLocalIndexPath() + getToday()
					+ "/")); //
	...
	public void WriteIndex(IndexModel model) {
		...
		try {
			...
			if (!model.coutent.equalsIgnoreCase("")) {
				Field fdcoutent = new TextField("coutent", model.coutent, Store.YES);
				doc.add(fdcoutent);
			}
			// 发布人
			if (!model.auther.equalsIgnoreCase("")) {
				...
				// doc.add(fdauther);
				// 发布人不分词
				Field fdautherstatic = new StringField("auther", model.auther, Store.YES);
			...
			try {
				...
				// 发布时间
				LongField fdreleasetime = new LongField("releasetime", DateTools.stringToTime(DateTools.dateToString(
						model.releasetime, Resolution.MILLISECOND)), Store.YES);
			...
			if (!model.qqgroupno.equalsIgnoreCase("")) {
				Field fdqqgroupno = new StringField("qqgroupno", model.qqgroupno, Store.YES);
				doc.add(fdqqgroupno);
			}

			// 境内：1，境外：2
			if (!model.jingnei.equalsIgnoreCase("")) {
				Field fdjingnei = new StringField("jingnei", model.jingnei, Store.YES);
				doc.add(fdjingnei);
			}

			// 区域代码
			if (!model.quyucode.equalsIgnoreCase("")) {
				Field fdquyucode = new StringField("quyucode", model.quyucode, Store.YES);
				doc.add(fdquyucode);
			}

			try {
				// 抓取时间
				LongField fdreleasetime = new LongField("spy", DateTools.stringToTime(DateTools.dateToString(
						new Date(), Resolution.MILLISECOND)), Store.YES);
