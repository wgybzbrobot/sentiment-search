...
import zx.soft.sent.lucene.common.ComparatorNameKeySort;
...
import zx.soft.sent.lucene.search.SearchIndex;
import zx.soft.sent.lucene.search.SearchModel;
import zx.soft.sent.lucene.utils.AnalyzerTool;
...
public class IndexServer {
	...
	public List<DayCountGroupModel> GetZhuanTiCountGroupModel(int storetype, String searchkey, Date start, Date end,
		...
		while (tempdate.before(end)) {
			...
			List<NameKeySort> modell = GetZhuanTiGroup(storetype, searchkey, tempdate, tenpendtime, typenoinlist,
	...
	public List<DayCountModel> GetZhuanTiCountModel(int storetype, int sortbyfbtime, String searchkey, Date start,
		...
		while (tempdate.before(end)) {
			...
			endcal.add(Calendar.HOUR, 24);
			Date tenpendtime = endcal.getTime();
			ReturnModel model = GetZhuanTiSearchAll(storetype, sortbyfbtime, searchkey, tempdate, tenpendtime,
					typenoinlist, sitenamelist, lyidinlist, typenooutlist, lyidoutList, qqqunlist, oldkeys, author,
					jingnei, listareain, listareaout, 1, 1);
	...
	public List<NameKeySort> GetZhuanTiGroup(int storetype, String searchkey, Date start, Date end,
		...
		try {
			...
			if (start.equals(end)) {
				...
				Query timequeryfilter = NumericRangeQuery.newLongRange("releasetime",
						DateTools.stringToTime(DateTools.dateToString(start, Resolution.MILLISECOND)),
						DateTools.stringToTime(DateTools.dateToString(end, Resolution.MILLISECOND)), true, false);
			...
			MultiFieldQueryParser qp = new MultiFieldQueryParser(Version.LUCENE_45, strfield,
			...
			TopGroups<Object> t = gSearch.search(searcher, flmain, blsearchquery, 0, 1000);// 设置返回数据
	...
	public ReturnModel GetZhuanTiSearchAll(int storetype, int sortbyfbtime, String searchkey, Date start, Date end,
		...
		try {
			...
			if (sortbyfbtime == 0) {// 不排序
				docsall = searcher.search(blsearchquery, flmain, pageindex * pagecount);
			} else if (sortbyfbtime == 1) {// 发布时间降序
				Sort str = new Sort();
				str.setSort(new SortField("releasetime", Type.LONG, true));
				docsall = searcher.search(blsearchquery, flmain, pageindex * pagecount, str);
			} else {// 发布时间升序
				Sort str = new Sort();
			...
			// 当前页数据
			for (int i = pageindex * pagecount - pagecount; i < docs.length && i < pageindex * pagecount; i++) {
				...
				if (doc.get("spy") != null) {
					Date dt = DateTools.stringToDate(DateTools.timeToString(Long.parseLong(doc.get("spy")),
							Resolution.SECOND));
	...
	public ReturnModel SearchAll(String stringkey, Date start, Date end, Date startspy, Date endspy, int typeNo,
		...
		try {
			...
			// 分词
			retmodel.analyzerlist = analyzerTool.analyzerTextToList(stringkey);
			// 初始化搜索
			SearchIndex se = SearchIndex.GetInstance();
			// 检索字段 标题 内容
			String[] strfield = { "title", "coutent" };
			...
			if (!start.equals(end)) {
				Query timequeryfilter = NumericRangeQuery.newLongRange("releasetime",
						DateTools.stringToTime(DateTools.dateToString(start, Resolution.MILLISECOND)),
						DateTools.stringToTime(DateTools.dateToString(end, Resolution.MILLISECOND)), true, true);
				blfilterquery.add(timequeryfilter, Occur.MUST);
			}
			// 监测时间不相等时增加检查时间过滤条件
			if (!startspy.equals(endspy)) {
				Query timequeryfilter = NumericRangeQuery.newLongRange("spy",
						DateTools.stringToTime(DateTools.dateToString(startspy, Resolution.MILLISECOND)),
						DateTools.stringToTime(DateTools.dateToString(endspy, Resolution.MILLISECOND)), true, true);
				blfilterquery.add(timequeryfilter, Occur.MUST);
			}
			...
			// 搜索
			TopDocs docsall = allSearcher.search(bfquery, flmain, pageindex * pagecount);
			// 计数
			ScoreDoc[] docs = docsall.scoreDocs;
	...
	public ReturnModel GetNetSearchAll(int sortbyfbtime, String searchkey, Date start, Date end,
		...
		try {
			// 分词
			retmodel.analyzerlist = analyzerTool.analyzerTextToList(searchkey);
			...
			if (searcher != null) {
				...
				for (ScoreDoc scoreDoc : docs) {
					...
					if (doc.get("releasetime") != null) {
						model.releasetime = DateTools.stringToDate(DateTools.timeToString(
								Long.parseLong(doc.get("releasetime")), Resolution.SECOND));
					}

					if (doc.get("spy") != null) {
						model.fetchTime = DateTools.stringToDate(DateTools.timeToString(Long.parseLong(doc.get("spy")),
								Resolution.SECOND));
					}
					modelList.add(model);
				}
